...
description: Takes a prospect's current tool stack and outputs the strongest competitive displacement angle specific to their configuration — the exact inefficiency their stack creates, two questions to surface it on a call, and a one-line email hook. Use when the user wants to displace a competitor or when a prospect's tech stack is known.
---
...
Which tool in their stack is most vulnerable to replacement, and why. Reference the exact limitation of that tool in the context of their full stack configuration. 2-3 sentences.
