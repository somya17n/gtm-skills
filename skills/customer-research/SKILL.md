...
> **Boundary:** This skill produces the research — themes, quotes, personas. For writing copy from that research, use `email-campaign`, `cold-email-writer`, or `landing-page`. For building audience segments, use `segment-builder`. For extracting the brand's own voice (not the customer's), use `brand-voice`.
