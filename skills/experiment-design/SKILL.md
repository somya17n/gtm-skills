...
> **Vocabulary:** Use "Experience" throughout — not "experiment" or "A/B test." This matches Intempt product terminology.

## Context

1. Check for `.agents/product-context.md` — if missing, ask the user to run `/gtm:product-context` first.
2. Load `references/bayesian-testing.md` for statistical design patterns and Thompson sampling details.
...
5. Read `.agents/product-context.md` to pull the north star metric and current baselines.
6. Formulate the hypothesis: "If [change], then [metric] will [direction] by [magnitude] because [mechanism]."
7. Define variants — control and one or more treatments. Describe what differs in each.
8. Select assignment strategy. Recommend Thompson sampling for most cases; fixed-allocation for simple two-variant tests.
9. Calculate statistical design:
   - Baseline conversion rate (from product context or user input)
   - Minimum detectable effect (MDE) — recommend 5-10% relative lift as default
   - Required sample size per variant
   - Estimated duration based on traffic
   - Confidence threshold (recommend 95% for decisions, 90% for iterations)
10. Define guardrails — metrics that must NOT degrade (e.g., unsubscribe rate, error rate).
11. Set exit criteria — when to stop: confidence threshold reached, max duration hit, or guardrail violated.
12. Specify holdout if measuring incremental lift beyond the experience itself.

## Output

13. Deliver the experience brief:
